<template>
  <a-modal
    :title="apiScope && apiScope.id > 0 ? 'Edit API Scope' : 'New API Scope'"
    :width="800"
    :visible="visible"
...
export default {
  ...
  methods: {
    ...
    saveProperty (rowKey) {
      ...
      if (target) {
        const target1 = this.apiScope.properties.find(item => item.rowKey === rowKey)
        if (target1) {
          Object.assign(target1, target)
        } else {
          this.apiScope.properties.push({ ...target })
        }
    ...
    deleteProperty (rowKey) {
      ...
      if (target) {
        ...
        this.apiScope.properties = this.apiScope.properties.filter(item => item.rowKey !== rowKey)
      }
    }
  },
  created () {
    ...
    this.$watch('model', () => {
      this.editing = false
      this.apiScope = this.model.data
      this.properties = this.apiScope.properties.map((item, index) => {
        item.rowKey = index
        return { ...item, rowKey: index }
      })

      this.form.setFieldsValue(pick(this.apiScope, fields))
    })
  }
}
</script>
