        ...
        <a-form layout="inline">
          <a-row :gutter="48">
            ...
            <template v-if="advanced">
              ...
              <a-col :md="12" :sm="24">
                <a-form-item label="ConsumedTime">
                  <a-date-picker
                    v-model="queryParam.consumedTimeBegin"
                    format="YYYY-MM-DD"
                    placeholder="Start"
                    style="width: 120px;"
                  />
                  ~
                  <a-date-picker
                    v-model="queryParam.consumedTimeEnd"
                    format="YYYY-MM-DD"
                    placeholder="End"
                    style="width: 120px;"
                  />
...
import { getPersistedGrantList, getPersistedGrantView, deletePersistedGrant } from '@/api/ids4admin'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return getPersistedGrantList(requestParameters)
          .then(result => {
            const data = {
  ...
  methods: {
    ...
    handleView (record) {
      const requestParameters = { key: record.key }
      getPersistedGrantView(requestParameters)
        .then(result => {
          this.visible = true
          this.mdl = result
        })
    ...
    handleDelete (record) {
      const requestParameters = { key: record.key }
      deletePersistedGrant(requestParameters)
        .then(result => {
