          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="ClientName">
                <a-input v-model="queryParam.clientName" placeholder="" />
              </a-form-item>
            </a-col>
            <template v-if="advanced">
              ...
              <a-col :md="12" :sm="24">
                <a-form-item label="LastAccessed">
                  <a-date-picker
                    v-model="queryParam.lastAccessedBegin"
                    format="YYYY-MM-DD"
                    placeholder="Start"
                    style="width: 120px;"
                  />
                  ~
                  <a-date-picker
                    v-model="queryParam.lastAccessedEnd"
                    format="YYYY-MM-DD"
                    placeholder="End"
                    style="width: 120px;"
                  />
...
import { getClientList, getClientInit, getClientView, insertClient, updateClient, deleteClient } from '@/api/ids4admin'
...
export default {
  ...
  methods: {
    ...
    handleEdit (record) {
      ...
      getClientView(requestParameters)
        .then(result => {
          this.visible = true
          this.mdl = result
        })
          ...
          if (data.id > 0) {
            updateClient(parameter)
              .then(result => {
                ...
                this.$message.info('Modify successffull.')
              })
              .catch(error => {
                this.confirmLoading = false
                notification.error({
            ...
            insertClient(parameter)
              .then(result => {
                ...
                this.$message.info('Add successffull.')
              })
              .catch(error => {
                this.confirmLoading = false
                notification.error({
    ...
    handleDelete (record) {
      const requestParameters = { id: record.id }
      deleteClient(requestParameters)
        .then(result => {
          this.$refs.table.refresh()
